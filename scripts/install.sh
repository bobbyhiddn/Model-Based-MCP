...
#   2. Eclipse SysON v2026.1.0 (SysML v2, Podman-based)
...
# Masternode: Debian 13 (trixie), x86_64, Java 21, Podman 5.x
# ============================================================================
...
usage() {
    ...
    echo ""
    echo "Installs Eclipse Papyrus Desktop, SysON (Podman), system deps,"
...
if command -v podman &>/dev/null; then
    ...
    warn "Podman not found. SysON installation will be skipped."
fi
...
if command -v podman &>/dev/null && podman compose version &>/dev/null 2>&1; then
    ...
    ok "Podman Compose: $(podman compose version 2>&1 | grep -v '>>>')"
elif command -v podman-compose &>/dev/null; then
    COMPOSE_CMD="podman-compose"
    ok "Podman Compose: $(podman-compose --version 2>&1)"
else
    warn "Podman Compose not found. SysON installation will be skipped."
    warn "Install with: pip install podman-compose  (or ensure docker-compose is available as backend)"
fi
...
# Check if apt is in a broken state (e.g. from a bad .deb install like Zoom)
if ! dpkg --configure -a --dry-run &>/dev/null 2>&1 || \
   apt-get check 2>&1 | grep -qi "broken\|unmet\|held broken"; then
    warn "apt appears to be in a broken state!"
    warn "This can happen from a bad .deb install (e.g. Zoom)."
    warn "Package installation may fail until this is fixed."
    echo ""
...
# ============================================================================
# STEP 3: Eclipse SysON (Podman)
# ============================================================================
step "Step 3/6: Eclipse SysON ${SYSON_VERSION} (Podman)"

if [[ -z "$COMPOSE_CMD" ]]; then
    warn "Skipping SysON — Podman Compose not available"
else
...
# Start: cd ~/Apps/SysON && podman compose up -d
# Stop:  cd ~/Apps/SysON && podman compose down
...
networks:
  syson:
COMPOSE_EOF
            ok "docker-compose.yml written to $SYSON_DIR/ (podman-compose compatible)"
        fi
    fi

    # Pre-pull images (don't start)
    if $DRY_RUN; then
        dry "podman pull postgres:15"
        dry "podman pull $SYSON_IMAGE"
    else
        info "Pre-pulling container images (this may take a few minutes)..."
        podman pull docker.io/library/postgres:15 2>&1 | tail -1
        podman pull "docker.io/${SYSON_IMAGE}" 2>&1 | tail -1
        ok "Container images pulled. Run 'syson-start' to launch."
    fi
fi
...
# Determine compose command (prefer podman)
if command -v podman &>/dev/null && podman compose version &>/dev/null 2>&1; then
    ...
    echo "Install podman or podman-compose."
    exit 1
fi
...
echo ""
echo "Check logs: podman compose -f $COMPOSE_FILE logs -f app"
...
echo -e "    syson-start       # Start SysON web app (Podman)"
