"""Model-Based-MCP server — MCP tools for creating SysML/MBSE diagrams and hierarchical canvas diagrams."""
...
# --- Constants ---
OUTPUT_DIR = Path(os.environ.get("MODEL_BASED_OUTPUT_DIR", Path.home() / ".rhode" / "canvas"))
TEMPLATES_DIR = Path(__file__).parent.parent.parent / "templates"

server = Server("model-based-mcp")
...
async def list_tools() -> list[Tool]:
    return [
        Tool(
            name="render_model",
            description=(
                "Render a model-based diagram from a YAML recipe string. "
                "Supports SysML/MBSE diagrams (BDD, IBD, Activity, etc.) and "
                "hierarchical format (networks/factories/machines/nodes) "
                ...
                "properties": {
                    "yaml_recipe": {
                        ...
                        "description": (
                            ...
                            "block, port, requirement, constraint, activity, signal\n"
        ...
        Tool(
            name="create_model",
            description=(
                "Create a model-based diagram from a structured description. "
                    ...
                    "nodes": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "type": {
                                    "type": "string",
                                    "enum": ["static", "input", "ai", "source", "output", "decision", "process", "default", "block", "port", "requirement", "constraint", "activity", "signal"],
...
async def call_tool(name: str, arguments: dict) -> list[TextContent | ImageContent]:
    if name == "render_model":
        return await _render_model(arguments)
    elif name == "create_model":
        return await _create_model(arguments)
...
async def _render_model(args: dict) -> list[TextContent]:
...
async def _create_model(args: dict) -> list[TextContent]:
    """Create a model-based diagram from structured input and render it."""
    _ensure_output_dir()
