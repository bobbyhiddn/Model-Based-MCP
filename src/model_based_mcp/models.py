"""
Data models for Model-Based-MCP — the canvas and SysML/MBSE ontology.

The ontology is a four-level hierarchy that models visual computation
...
This module defines the **node type system** — the original eight Canvas-MCP
semantic types plus six SysML/MBSE types for model-based systems engineering:

    Canvas types (inherited):
    ...
    SysML/MBSE types (new):
    block       — SysML block (structural element in BDD/IBD)
    port        — SysML port (interface point on a block)
    requirement — SysML requirement (a system requirement)
    constraint  — SysML constraint (a parametric constraint)
    activity    — SysML activity (an action or behavior)
    signal      — SysML signal (an event or message)
"""
...
NODE_STYLES: dict[str, NodeStyle] = {
    ...
    # SysML/MBSE types
    "block":       NodeStyle(border_color="#42A5F5"),  # Light Blue — structural element
    "port":        NodeStyle(border_color="#AB47BC"),  # Light Purple — interface point
    "requirement": NodeStyle(border_color="#66BB6A"),  # Light Green — system requirement
    "constraint":  NodeStyle(border_color="#EF5350"),  # Light Red — parametric constraint
    "activity":    NodeStyle(border_color="#26C6DA"),  # Teal — action/behavior
    "signal":      NodeStyle(border_color="#FFCA28"),  # Yellow — event/message
}
